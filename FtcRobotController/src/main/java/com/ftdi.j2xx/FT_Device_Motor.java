...
public class FT_Device_Motor extends FT_Device {
    ...
    */
    ...
    protected final byte[] RECEIVE_SYNC_COMMAND_30 = { 51, -52, -128, 0, (byte)30};
    ...
    public int write(byte[] data, int length, boolean wait)
    {
        ...
        if (data[0] == WRITE_COMMAND[0] && data[2] == WRITE_COMMAND[2]) {  // WRITE_COMMAND
            ...
            if (data[4] == (byte)30 ) {
                ...
                super.sendPacketToPC(data, 5, 30);
...
//                Log.v("Legacy", "WRITE: Delta Time = " + mDeltaWriteTime);
            ...
            } else if (data[4] == (byte)30) { // Android asks for 94 bytes, full read of device
                queueUpForReadFromPhone(RECEIVE_SYNC_COMMAND_30);  // Send receive sync loop back
